...
class paginator
{
    ...
    public function __construct($pdo, $newsPerPage, $pageNumber)
    {
        ...
        $countNews = $this->pdo->query("SELECT COUNT(`news`) FROM `news`");
        $resOfCountNews = $countNews->fetchColumn();
        $this->numberOfPages = ceil($resOfCountNews / $this->newsPerPage);
        ($this->pageNumber > $this->numberOfPages) ? 
            $this->pageNumber = print_r("Такой страницы не существует") : 
    ...
    {
        $firstNews = $this->newsPerPage * $this->pageNumber - $this->newsPerPage;
        $this->preNewsOnPage = $this->pdo->prepare("SELECT * FROM `news` LIMIT :firstNews, :newsPerPage");
        $this->preNewsOnPage->bindParam(':firstNews', $firstNews, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT,4);
        $this->preNewsOnPage->bindParam(':newsPerPage', $newsPerPage, PDO::PARAM_INT | PDO::PARAM_INPUT_OUTPUT,4);      
...
foreach ($newPage->$this->newsOnPage as $value) {
    echo '<p>' . $value['news'] . '</p>';
}         
